  ...
  const onFinish = async (values) => {
    ...
    if (selectedRows?.length === 1) {
      await updateUser({
        ...
        .catch((error) => {
          // openNotification('error', ' エラー', error?.message)
          Notification({
            ...
            validator(_, value) {
              if (value?.length > 0 && value?.length < 4) {
                return Promise.reject(
                  new Error('ユーザーIDは4文字以上設定してください。')
                  // setErrState(true)
                )
              }
...

      {
        ...
        >
          ...
          >
            ...
            <Option value={ROLE_CLIENT}>{translation.TEXT_CLIENT}</Option>
          </Select>
        </FormAddUser.Item>
      }
